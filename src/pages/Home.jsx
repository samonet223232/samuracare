  ...
  return (
    <main>
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-bl from-white/65 via-olive-50/60 to-olive-100/40" />
        <div className="relative z-10 text-center max-w-3xl mx-auto px-6">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center animate-fade-up">
            ...
            <Link to="/articles" className="border-2 border-olive-400 text-olive-600 bg-white/70 backdrop-blur-sm px-8 py-4 rounded-full text-sm font-sans font-medium hover:bg-olive-500 hover:text-white hover:border-olive-500 transition-all duration-300">
              اقرأي المقالات
            </Link>
          </div>
        </div>
      ...
      {/* Trust bar */}
      <FadeInSection>
        <section className="bg-olive-50 border-y border-olive-200 py-8">
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <FadeInSection className="lg:col-span-2">
              <Link to={`/articles/${featuredArticles[0].slug}`} className="group block bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-olive-100 hover:border-olive-300 h-full">
                ...
                <div className="p-7">
                  <span className="text-xs text-olive-500 font-sans bg-olive-50 px-2.5 py-1 rounded-full">{featuredArticles[0].category}</span>
            ...
            <div className="flex flex-col gap-5">
              {featuredArticles.slice(1, 4).map((article, i) => (
                <FadeInSection key={article.id} delay={i * 80}>
                  <Link to={`/articles/${article.slug}`} className="group flex gap-4 bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all border border-olive-100 hover:border-olive-300">
                    <div className="relative w-28 shrink-0 overflow-hidden">
                      <img src={article.image} alt={article.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
                    </div>
                    <div className="p-4">
                      <span className="text-xs text-olive-500 bg-olive-50 px-2 py-0.5 rounded-full font-sans">{article.category}</span>
          ...
          <FadeInSection>
            <div className="text-center mt-12">
              <Link to="/articles" className="inline-flex items-center gap-2 border-2 border-olive-400 text-olive-600 px-8 py-3.5 rounded-full text-sm font-sans font-medium hover:bg-olive-500 hover:text-white hover:border-olive-500 transition-all">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-14 items-center">
            <FadeInSection>
              <div className="relative">
                ...
                <div className="absolute -bottom-5 -left-5 bg-olive-500 text-white rounded-2xl p-5 shadow-xl max-w-[190px]">
            ...
            <FadeInSection delay={150}>
              <div>
                ...
                <Link to="/about" className="inline-flex items-center gap-2 bg-olive-500 text-white px-7 py-3.5 rounded-full text-sm font-sans font-medium hover:bg-olive-600 transition-colors shadow-md shadow-olive-200">
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            {guideCategories.map((cat, i) => (
              <FadeInSection key={cat.id} delay={i * 100}>
                <Link to={`/guide?category=${cat.id}`} className="group block bg-neutral-800 hover:bg-olive-700 rounded-2xl p-8 text-center transition-all duration-300 border border-neutral-700 hover:border-olive-500">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-12">
            {featuredGuide.map((entry, i) => (
              <FadeInSection key={entry.id} delay={i * 80}>
                <Link to={`/guide/${entry.slug}`} className="group flex items-center gap-4 bg-neutral-800 rounded-xl p-4 hover:bg-olive-600 transition-colors duration-300 border border-neutral-700 hover:border-olive-500">
          ...
          <FadeInSection>
            <div className="text-center">
              <Link to="/guide" className="inline-flex items-center gap-2 border-2 border-olive-400 text-olive-300 px-8 py-3.5 rounded-full text-sm font-sans font-medium hover:bg-olive-500 hover:text-white hover:border-olive-500 transition-all">
      ...
      {/* Newsletter */}
      <FadeInSection>
        <section className="bg-olive-100 border-y border-olive-200 py-20 px-6">
