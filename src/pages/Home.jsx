  ...
  return (
    <main>
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-bl from-white/70 via-cream-100/50 to-olive-50/60" />
        <div className="relative z-10 text-center max-w-3xl mx-auto px-6">
          <p className="text-xs font-sans font-medium tracking-widest text-olive-500 mb-4 animate-fade-in">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center animate-fade-up">
            <Link to="/guide" className="bg-olive-500 text-white px-8 py-4 rounded-full text-sm font-sans font-medium hover:bg-olive-600 transition-colors duration-300">
              استكشفي الدليل
            </Link>
            <Link to="/articles" className="border border-neutral-400 text-neutral-700 px-8 py-4 rounded-full text-sm font-sans font-medium hover:border-olive-300 hover:text-olive-500 transition-colors duration-300">
              اقرأي المقالات
            </Link>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <FadeInSection>
            <div className="text-center mb-14">
              <p className="text-xs font-sans tracking-widest text-olive-400 mb-3">معرفة الجمال</p>
              <h2 className="font-serif text-4xl md:text-5xl font-semibold text-neutral-800">المقالات والأدلة</h2>
              <div className="mt-4 w-12 h-px bg-olive-300 mx-auto" />
            </div>
          </FadeInSection>
              ...
              {featuredArticles.slice(1, 4).map((article, i) => (
                <FadeInSection key={article.id} delay={i * 80}>
                  <Link to={`/articles/${article.slug}`} className="group flex gap-4 bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <div className="p-4">
                      <span className="text-xs text-olive-400 font-sans">{article.category}</span>
          ...
          <FadeInSection>
            <div className="text-center mt-12">
              <Link to="/articles" className="inline-flex items-center gap-2 border border-neutral-300 text-neutral-700 px-8 py-3.5 rounded-full text-sm font-sans font-medium hover:border-olive-300 hover:text-olive-400 transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-14 items-center">
          <FadeInSection>
            <div className="relative">
              ...
              <div className="absolute -bottom-5 -left-5 bg-olive-100 rounded-2xl p-5 shadow-lg max-w-[190px]">
                <p className="font-serif text-3xl font-semibold text-olive-600">+8</p>
          ...
          <FadeInSection delay={150}>
            <div>
              ...
              <Link to="/about" className="inline-flex items-center gap-2 bg-cream-200 text-neutral-800 px-7 py-3.5 rounded-full text-sm font-sans font-medium hover:bg-olive-100 transition-colors">
        ...
        <div className="max-w-7xl mx-auto px-6">
          <FadeInSection>
            <div className="text-center mb-14">
              ...
              <div className="mt-5 w-12 h-px bg-olive-300 mx-auto" />
            </div>
          </FadeInSection>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            {guideCategories.map((cat, i) => (
              <FadeInSection key={cat.id} delay={i * 100}>
                <Link to={`/guide?category=${cat.id}`} className="group block bg-neutral-800 hover:bg-neutral-700 rounded-2xl p-8 text-center transition-colors duration-300 border border-neutral-700 hover:border-olive-500">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-12">
            {featuredGuide.map((entry, i) => (
              <FadeInSection key={entry.id} delay={i * 80}>
                <Link to={`/guide/${entry.slug}`} className="group flex items-center gap-4 bg-neutral-800 rounded-xl p-4 hover:bg-neutral-700 transition-colors border border-neutral-700 hover:border-olive-300">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-olive-300 mt-1">{entry.taglineAr}</p>
                  </div>
                </Link>
              </FadeInSection>
            ))}
          ...
          <FadeInSection>
            <div className="text-center">
              <Link to="/guide" className="inline-flex items-center gap-2 border border-neutral-500 text-neutral-200 px-8 py-3.5 rounded-full text-sm font-sans font-medium hover:border-olive-300 hover:text-olive-300 transition-colors">
      ...
      {/* Newsletter */}
      <FadeInSection>
        <section className="bg-olive-50 border-y border-olive-100 py-20 px-6">
          <div className="max-w-2xl mx-auto text-center">
            <p className="text-xs font-sans tracking-widest text-olive-500 mb-3">ابقي على اطلاع</p>
            ...
            <form className="flex flex-col sm:flex-row gap-3 justify-center" onSubmit={(e) => e.preventDefault()}>
              <input
                ...
                className="flex-1 max-w-xs px-5 py-3 rounded-full border border-cream-300 bg-white text-sm font-sans focus:outline-none focus:ring-2 focus:ring-olive-200"
              />
              <button type="submit" className="bg-olive-500 text-white px-6 py-3 rounded-full text-sm font-sans font-medium hover:bg-olive-600 transition-colors">
                اشتركي
              </button>
            </form>
