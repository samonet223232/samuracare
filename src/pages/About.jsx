  ...
  return (
    <main className="pt-24">
      ...
      <section className="relative min-h-[60vh] flex items-center overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-l from-olive-900/85 via-olive-800/60 to-transparent" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
            <FadeInSection>
              <div className="relative">
                ...
                <div className="absolute -bottom-6 -right-6 bg-olive-500 text-white rounded-2xl p-6 shadow-xl max-w-[200px]">
      ...
      {/* Values */}
      <section className="bg-olive-100 py-24 px-6 border-y border-olive-200">
            ...
            {values.map((v, i) => (
              <FadeInSection key={v.title} delay={i * 80}>
                <div className="bg-white rounded-2xl p-7 text-center shadow-sm border border-olive-200 h-full hover:border-olive-400 transition-colors">
                  <div className="w-12 h-12 bg-olive-500 rounded-full flex items-center justify-center mx-auto mb-4 text-white">
                    {v.icon}
                  </div>
      ...
      <section className="bg-white py-24 px-6">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="relative pr-8 border-r-2 border-olive-300">
            {timeline.map((item, i) => (
              <FadeInSection key={item.year} delay={i * 80}>
                <div className="mb-8 relative">
                  <div className="absolute -right-[2.6rem] w-5 h-5 bg-olive-500 rounded-full border-4 border-white shadow-sm" />
      ...
      {/* CTA */}
      <FadeInSection>
        <section className="bg-olive-600 py-20 px-6 text-center">
          <div className="max-w-2xl mx-auto">
            <p className="text-xs font-sans tracking-widest text-olive-200 mb-4">انضمي إلى الرحلة</p>
            <h2 className="font-serif text-3xl md:text-4xl font-semibold text-white mb-5">
              الجمال ممارسة، لا مجرّد منتج
            </h2>
            <p className="text-olive-100 text-base mb-8 leading-relaxed">
              سواء كنتِ تقرأين مقالاتنا أو تتعمّقين في دليل الجمال — يسعدنا وجودكِ هنا.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link to="/articles" className="bg-white text-olive-600 px-7 py-3.5 rounded-full text-sm font-sans font-medium hover:bg-olive-50 transition-colors shadow-md">
                اقرأي المقالات
              </Link>
              <Link to="/guide" className="border-2 border-white/60 text-white px-7 py-3.5 rounded-full text-sm font-sans font-medium hover:border-white hover:bg-olive-700 transition-all flex items-center gap-2 justify-center">
      ...
      <FadeInSection>
        <section className="bg-olive-50 py-6 px-6 border-t border-olive-200">
