  ...
  return (
    <main className="pt-24">
          ...
          {categories.map((cat) => (
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-xs font-sans font-medium transition-all duration-200 ${
                active === cat ? 'bg-olive-500 text-white shadow-sm' : 'bg-white text-neutral-600 border border-neutral-200 hover:border-neutral-400'
              }`}
            >
              {cat}
            </button>
      ...
      <section className="max-w-7xl mx-auto px-6 py-16">
        ...
        {featured && (
          <FadeInSection>
            <Link to={`/articles/${featured.slug}`} className="group block bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all mb-12 border border-neutral-100">
              <div className="grid grid-cols-1 md:grid-cols-2">
                <div className="relative overflow-hidden h-72 md:h-auto">
                  <img src={featured.image} alt={featured.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                  <span className="absolute top-4 right-4 bg-neutral-900 text-white text-xs font-sans font-medium px-3 py-1 rounded-full">مميّز</span>
                </div>
                <div className="p-8 md:p-12 flex flex-col justify-center">
                  <span className="text-xs text-olive-500 font-sans inline-block mb-4 w-fit">{featured.category}</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-7">
          {rest.map((article, i) => (
            <FadeInSection key={article.id} delay={i * 80}>
              <Link to={`/articles/${article.slug}`} className="group block bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-all h-full border border-neutral-100">
